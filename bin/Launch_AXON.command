...
if [ -d "core/AXON_VAULT" ]; then cp -R core/AXON_VAULT "backups/AXON_VAULT_$TIMESTAMP"; fi

# Cleanup old legacy folders if they exist
if [ -d "pb_data" ]; then mv pb_data "backups/pb_data_root_$TIMESTAMP"; fi
if [ -f "core/axon_v1.db" ]; then cp core/axon_v1.db "backups/axon_v1_$TIMESTAMP.db"; fi
